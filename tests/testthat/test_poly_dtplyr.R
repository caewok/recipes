...
context("dtplyr: Polynomial features")
...
test_that('correct basis functions', {
  ...
  with_poly <- prep(with_poly, training = biomass_tr_dt, verbose = FALSE)

  expect_equal(exp_tidy_un, tidy(with_poly, number = 1))

  with_poly_pred_tr <- bake(with_poly, new_data = biomass_tr_dt)
  with_poly_pred_te <- bake(with_poly, new_data = biomass_te_dt)
...
test_that('printing', {
  ...
  expect_output(prep(with_poly, training = biomass_tr_dt, verbose = TRUE))
})
