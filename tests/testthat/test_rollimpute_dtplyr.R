...
context("dtplyr: Rolling imputation")
...
test_that('imputation values with 3-pt mean', {
  ...
  three_pt <- recipe(~ . , data = example_data_dt) %>%
    update_role(day, new_role = "time_index") %>%
...
test_that('bad args', {
  ...
  example_data_dt <- example_data_dt %>%
    dplyr::mutate(x4 = 1:12)
  expect_error(
    recipe( ~ . , data = example_data_dt) %>%
      update_role(day, new_role = "time_index") %>%
      step_rollimpute(all_predictors(), window = 3) %>%
...
test_that('printing', {
  ...
  expect_output(prep(seven_pt, training = example_data_dt, verbose = TRUE))
})
