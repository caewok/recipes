...
rec <- recipe( ~ x1 + x2 + x3 + x4 + x5, data = ex_dat_dt)
...
test_that('1:many', {
  ...
  rec1 <- prep(rec1, ex_dat_dt, verbose = FALSE)
  obs1 <- bake(rec1, ex_dat_dt)
  res1 <- tibble(
...
test_that('many:1', {
  ...
  rec2 <- prep(rec2, ex_dat_dt, verbose = FALSE)
  obs2 <- bake(rec2, ex_dat_dt)
  res2 <- tibble(
...
test_that('many:many', {
  ...
  rec3 <- prep(rec3, ex_dat_dt, verbose = FALSE)
  obs3 <- bake(rec3, ex_dat_dt)
  res3 <- tibble(
    x2_o_x1   = ex_dat$x2/ex_dat$x1,
    x3_o_x1   = ex_dat$x3/ex_dat$x1,
    x4_o_x1   = ex_dat$x4/ex_dat$x1,
...
test_that('wrong type', {
  ...
  expect_error(prep(rec4, ex_dat_dt, verbose = FALSE))

  rec5 <- rec %>%
    step_ratio(all_predictors(), denom = denom_vars(x1))
  expect_error(prep(rec5, ex_dat_dt, verbose = FALSE))

  rec6 <- rec %>%
    step_ratio(all_predictors(), denom = denom_vars(all_predictors()))
  expect_error(prep(rec6, ex_dat_dt, verbose = FALSE))
})
...
test_that('printing', {
  ...
  expect_output(prep(rec3, training = ex_dat_dt, verbose = TRUE))
})
