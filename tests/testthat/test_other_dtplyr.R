...
context("dtplyr: Pooling to other category")
...
okc_tr_dt <- lazy_dt(okc_tr)
okc_te_dt <- lazy_dt(okc_te)

rec <- recipe(~ diet + location, data = okc_tr_dt)
...
test_that('default inputs', {
  ...
  others <- prep(others, training = okc_tr_dt)
  others_te <- bake(others, new_data = okc_te_dt)
...
test_that('low threshold - no removals', {
  ...
  others <- prep(others, training = okc_tr_dt, strings_as_factors = FALSE)
  others_te <- bake(others, new_data = okc_te_dt)
...
test_that('novel levels', {
  ...
  novel_level <- recipe(y ~ ., data = training_dt) %>%
    step_other(x1)

  novel_level <- prep(novel_level, training = training_dt)
  new_results <- bake(novel_level, new_data = testing_dt)
  orig_results <- bake(novel_level, new_data = training_dt)
...
test_that("'other' already in use", {
  ...
  expect_error(
    prep(others, training = okc_tr_dt, strings_as_factors = FALSE)
  )
})
...
test_that('printing', {
  ...
  expect_output(prep(rec, training = okc_tr_dt, verbose = TRUE))
})
...
test_that(
  ...
  code = {
    ...
    fake_data_dt <- lazy_dt(fake_data)

    rec <- recipe(~ test_factor, data = fake_data_dt)
...
test_that('issue #415 -  strings to factor conversion', {
  ...
  prepped <- prep(trans_recipe, iris_dt)

  iris_no_outcome <- iris
  iris_no_outcome["Species"] <- NULL
  iris_no_outcome_dt <- lazy_dt(iris_no_outcome)

  expect_error(
    res <- bake(prepped, iris_no_outcome_dt),
    regex = NA
  )
