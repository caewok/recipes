...
context("dtplyr: Ordinal data to scores")
...
ex_miss_dt <- lazy_dt(ex_miss)

score <- function(x) as.numeric(x)^2

test_that('linear scores', {
  rec1 <- recipe(~ ., data = ex_dat_dt) %>%
    step_ordinalscore(starts_with("ord"))
  rec1 <- prep(rec1, training = ex_dat_dt,
               strings_as_factors = FALSE, verbose = FALSE)
  rec1_scores <- bake(rec1, new_data = ex_dat_dt)
  rec1_scores_NA <- bake(rec1, new_data = ex_miss_dt)
...
test_that('nonlinear scores', {
  rec2 <- recipe(~ ., data = ex_dat_dt) %>%
    step_ordinalscore(starts_with("ord"),
                      convert = score)
  rec2 <- prep(rec2, training = ex_dat_dt,
               strings_as_factors = FALSE, verbose = FALSE)
  rec2_scores <- bake(rec2, new_data = ex_dat_dt)
  rec2_scores_NA <- bake(rec2, new_data = ex_miss_dt)
...
test_that('bad spec', {
  rec3 <- recipe(~ ., data = ex_dat_dt) %>%
    step_ordinalscore(everything())
  expect_error(prep(rec3, training = ex_dat_dt, verbose = FALSE))
  rec4 <- recipe(~ ., data = ex_dat_dt)
  expect_error(rec4 %>% step_ordinalscore())
})


test_that('printing', {
  rec5 <- recipe(~ ., data = ex_dat_dt) %>%
    step_ordinalscore(starts_with("ord"))
  expect_output(print(rec5))
  expect_output(prep(rec5, training = ex_dat_dt, verbose = TRUE))
})
