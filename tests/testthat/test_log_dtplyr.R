...
context("dtplyr: Log transformation")
...
test_that('signed arg', {
  ...
  ex_with_neg_dt <- lazy_dt(ex_with_neg)

  rec <- recipe(ex_with_neg_dt, ~x) %>%
    step_log(x, signed = TRUE) %>%
    prep()
  rec2 <- recipe(ex_with_neg_dt, ~x) %>%
    step_log(x, offset = 2, signed = TRUE)
  expect_equal(bake(rec, ex_with_neg_dt), tibble(x = c(-2, 0, 0, 2)))
