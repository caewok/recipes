...
biomass_tr <- biomass_dt %>% dplyr::filter(dataset == "Training")
biomass_te <- biomass_dt %>% dplyr::filter(dataset == "Testing")

test_that("Recipe correctly identifies output variable", {
  raw_recipe <- recipe(HHV ~ ., data = biomass_dt)
  ...
  expect_true(all(var_info$role[-outcome_ind] == rep("predictor", ncol(biomass_dt) - 1)))
})

test_that("Recipe fails on in-line functions", {
  expect_error(recipe(HHV ~ log(nitrogen), data = biomass_dt))
  expect_error(recipe(HHV ~ (.)^2, data = biomass_dt))
  expect_error(recipe(HHV ~ nitrogen  + sulfur  + nitrogen:sulfur, data = biomass_dt))
  expect_error(recipe(HHV ~ nitrogen^2, data = biomass_dt))
})
...
test_that("return character or factor values", {
  ...
  centered_char <- prep(centered, training = biomass_dt, strings_as_factors = FALSE)
  char_var <- bake(centered_char, new_data = head(biomass_dt))
  expect_equal(class(char_var$sample), "character")

  centered_fac <- prep(centered, training = biomass_dt, strings_as_factors = TRUE)
  fac_var <- bake(centered_fac, new_data = head(biomass_dt))
  expect_equal(class(fac_var$sample), "factor")

  # weird error about not finding "." function when using dplyr::pull
  # only when running Test Package; not when running Run Tests or when running interactively
  # sample_val <- pull(biomass_dt, .data[["sample"]])
  # sample_val <- biomass_dt %>% dplyr::select(sample) %>% as_tibble %>% unlist()
  sample_val <- biomass_dt %>% compute() %>% dplyr::pull(sample)
  expect_equal(levels(fac_var$sample), sort(unique(sample_val)))
})


test_that("Using prepare", {
  expect_error(prepare(recipe(HHV ~ ., data = biomass_dt),
                       training = biomass_dt),
...
test_that("Multiple variables on lhs of formula", {
  ...
  expect_silent(multi_1 <- recipe(Petal.Width + Species ~ ., data = iris_dt))
  ...
  iris_dt$Species <- as.character(iris_dt$Species)
  expect_silent(multi_2 <- recipe(Petal.Width + Species ~ ., data = iris_dt))
  expect_equal(multi_2$var_info$variable[multi_2$var_info$role == "outcome"],
               colnames(iris_dt)[4:5])
  expect_equal(multi_2$var_info$variable[multi_2$var_info$role == "predictor"],
               colnames(iris_dt)[1:3])
...
test_that("bake without prep", {
  sp_signed <-  recipe(HHV ~ ., data = biomass_dt) %>%
    step_center(all_predictors()) %>%
...
test_that("bake without newdata", {
  rec <-  recipe(HHV ~ ., data = biomass_dt) %>%
    step_center(all_numeric()) %>%
    step_scale(all_numeric()) %>%
    prep(training = biomass_dt)

  expect_error(bake(rec, newdata = biomass_dt))
})

test_that("`juice()` returns a 0 column / N row tibble when a selection returns no columns", {
  rec <- recipe(~ ., data = iris_dt)
  rec <- prep(rec, iris_dt)

  expect_equal(
    juice(rec, all_outcomes(), composition = "tibble"),
...
test_that("`bake(new_data = NULL)` same as `juice()`", {
  ...
  roasted <- bake(rec, new_data = mtcars_dt)
