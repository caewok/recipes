...
test_that('correct basis functions', {
  ...
  with_bs <- prep(with_bs, training = biomass_tr_dt, verbose = FALSE)

  with_bs_pred_tr <- bake(with_bs, new_data = biomass_tr_dt)
  with_bs_pred_te <- bake(with_bs, new_data = biomass_te_dt)
...
test_that('printing', {
  ...
  expect_output(prep(with_bs, training = biomass_tr_dt, verbose = TRUE))
})
