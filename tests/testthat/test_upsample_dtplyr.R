...
test_that('basic usage', {
  ...
  rec1_p <- prep(rec1, training = iris2_dt)
  ...
  te_xtab <- table(bake(rec1_p, new_data = iris2_dt)$Species, useNA = "always")
  ...
  expect_warning(prep(rec1, training = iris2_dt), NA)
})
...
test_that('ratio value', {
  ...
  rec2_p <- prep(rec2, training = iris2_dt)

  tr_xtab <- table(juice(rec2_p)$Species, useNA = "always")
  te_xtab <- table(bake(rec2_p, new_data = iris2_dt)$Species, useNA = "always")
  og_xtab <- table(iris2$Species, useNA = "always")
...
test_that('no skipping', {
  ...
  rec3_p <- prep(rec3, training = iris2_dt)

  tr_xtab <- table(juice(rec3_p)$Species, useNA = "always")
  te_xtab <- table(bake(rec3_p, new_data = iris2_dt)$Species, useNA = "always")
  og_xtab <- table(iris2$Species, useNA = "always")
...
test_that('printing', {
  ...
  expect_output(prep(rec4, training = iris2_dt, verbose = TRUE))
})
