...
context("dtplyr: Interaction creation")
...
dat_tr_dt <- lazy_dt(dat_tr)
dat_te_dt <- lazy_dt(dat_te)

rec <- recipe(y ~ .,  data = dat_tr_dt)

test_that('non-factor variables with dot', {
  int_rec <- rec %>% step_interact( ~ (. - y- z) ^ 3 , sep = ":")
  int_rec_trained <-
    prep(int_rec, training = dat_tr_dt, verbose = FALSE)

  te_new <-
    bake(int_rec_trained, new_data = dat_te_dt, all_predictors(), -all_nominal())
...
test_that('non-factor variables with specific variables', {
  ...
  og_terms <- terms( ~ x1 + x2 + x3 + x4 + x5 +
                       x1:x2 + x3:x4:x5, data = dat_te_dt)
  te_og <- model.matrix(og_terms, data = dat_te_dt)[,-1]
  te_og <- te_og[, sort(colnames(te_og))]
...
# dtplyr does not (yet) support predicates
# test_that("using where() works", {
#   ex_rec <- rec %>%
#     step_dummy(z) %>%
#     step_interact( ~ where(is.numeric):x1)
#
#   x <- prep(ex_rec, dat_tr_dt)
#
...
test_that('printing', {
  ...
  expect_output(prep(int_rec, training = dat_tr_dt, verbose = TRUE))
})
...
test_that('missing columns', {
  no_fail <-
    ...
    step_rm(x1) %>%
    step_interact(~x1:x2)
  no_fail_rec <- expect_warning(prep(no_fail, dat_tr_dt))
  ...
  one_int <-
    ...
    step_interact(~x1:x2) %>%
    step_interact(~x3:x2)
  one_int_rec <- expect_warning(prep(one_int, dat_tr_dt))
  ...
  with_selectors <-
    ...
    step_interact(~x3:x2)
  expect_warning(prep(with_selectors, dat_tr_dt), regexp = NA)
})
