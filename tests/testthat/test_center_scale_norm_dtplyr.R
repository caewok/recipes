...
context("dtplyr: Testing center and scale")
...
test_that('correct means and std devs', {
  ...
  standardized_trained <- prep(standardized, training = biomass_dt)
...
test_that('training in stages', {
  ...
  at_once_trained <- prep(at_once, training = biomass_dt)

  ## not train in stages
  center_first <- rec %>%
    step_center(carbon, hydrogen, oxygen, nitrogen, sulfur, id = "center")
  center_first_trained <- prep(center_first, training = biomass_dt)
  in_stages <- center_first_trained %>%
    step_scale(carbon, hydrogen, oxygen, nitrogen, sulfur, id = "scale")
  in_stages_trained <- prep(in_stages)
  in_stages_retrained <-
    prep(in_stages, training = biomass_dt, fresh = TRUE)

  # for dtplyr, checking equality for the template dtplyr table is likely to fail
  # simply because the equality check is picking up minor variations based on what got called first.
  trained_names_to_check <- setdiff(names(at_once_trained), "template")
  expect_equal(at_once_trained[trained_names_to_check], in_stages_trained[trained_names_to_check])
  expect_equal(at_once_trained[trained_names_to_check], in_stages_retrained[trained_names_to_check])
})
...
test_that('single predictor', {
  ...
  standardized_trained <- prep(standardized, training = biomass_dt)
  results <- bake(standardized_trained, biomass_dt, composition = "dtplyr")
...
test_that('printing', {
  ...
  expect_output(prep(standardized, training = biomass_dt, verbose = TRUE))
})
