...
test_that('correct kernel PCA values', {
  ...
  kpca_trained <- prep(kpca_rec, training = tr_dat_dt, verbose = FALSE)

  pca_pred <- bake(kpca_trained, new_data = te_dat_dt, all_predictors())
...
test_that('printing', {
  ...
  expect_output(prep(kpca_rec, training = tr_dat_dt, verbose = TRUE))
})
