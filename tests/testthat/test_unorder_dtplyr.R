...
context("dtplyr: Unordering data")
...
examples_dt <- lazy_dt(examples)
rec <- recipe(~ X1 + X2, data = examples_dt)

test_that('correct var', {
  rec1 <- rec %>% step_unorder(X2)

  rec1_trained <- prep(rec1, training = examples_dt, verbose = FALSE)
  rec1_trans <- bake(rec1_trained, new_data = examples_dt)
...
test_that('wrong vars', {
  ...
  expect_warning(prep(rec2, training = examples_dt, verbose = FALSE))
  rec3 <- rec %>% step_unorder(X1)
  expect_error(prep(rec3, training = examples_dt, verbose = FALSE))
})

test_that('printing', {
  rec4 <- rec %>% step_unorder(X2)
  expect_output(print(rec4))
  expect_output(prep(rec4, training = examples_dt, verbose = TRUE))
})
