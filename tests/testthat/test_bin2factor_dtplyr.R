...
context("dtplyr::binary to factor conversion")

library(modeldata)
data(covers)
covers_dt <- lazy_dt(covers)

rec <- recipe(~ description, covers_dt) %>%
...
test_that('printing', {
  ...
  expect_output(prep(rec2, training = covers_dt, verbose = TRUE))
})


test_that('choose reference level', {
  rec4 <- rec %>% step_bin2factor(rocks, ref_first = FALSE)
  rec4 <- prep(rec4, training = covers_dt)
  res4 <- bake(rec4, new_data = covers_dt)
