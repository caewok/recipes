...
test_that('imputation models', {
  ...
  expect_warning(imputed_trained <- prep(imputed, training = biomass_dt, verbose = FALSE))
...
test_that('printing', {
  ...
  expect_output(expect_warning(prep(imputed, training = biomass_dt, verbose = TRUE)))
})
