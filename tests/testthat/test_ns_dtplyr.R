...
test_that('correct basis functions', {
  ...
  with_ns <- prep(with_ns, training = biomass_tr_dt, verbose = FALSE)

  with_ns_pred_tr <- bake(with_ns, new_data = biomass_tr_dt, composition = "tibble")
  with_ns_pred_te <- bake(with_ns, new_data = biomass_te_dt, composition = "tibble")

  # expected, using the original
...
test_that('printing', {
  ...
  expect_output(prep(with_ns, training = biomass_tr_dt, verbose = TRUE))
})
