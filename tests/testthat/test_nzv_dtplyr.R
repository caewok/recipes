...
context("dtplyr: Near zero variance filter")
...
test_that('nzv filtering', {
  ...
  filtering_trained <- prep(filtering, training = dat_dt, verbose = FALSE)
...
test_that('printing', {
  ...
  expect_output(prep(rec, training = dat_dt, verbose = TRUE))
})
