context("dtplyr: PLS (new objects)")
...
cell_tr_dt <- lazy_dt(cell_tr)
cell_te_dt <- lazy_dt(cell_te)
...
test_that('PLS, dense loadings', {
  ...
  rec <- recipe(HHV ~ ., data = biom_tr_dt) %>%
...
test_that('PLS-DA, dense loadings', {
  skip_if_not_installed("mixOmics")
  rec <- recipe(class ~ ., data = cell_tr_dt) %>%
...
test_that('No PLS', {
  ...
  te_new <- bake(rec, cell_te_dt, all_predictors())
