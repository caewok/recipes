...
context("dtplyr: step_cut function")

test_that("step_cut throws error on non-numerics", {
  x <- tibble(num_var = 1:3, cat_var = c("1", "2" ,"3"))
  x_dt <- lazy_dt(x)

  expect_error(recipe(x_dt) %>% step_cut(num_var, breaks = 2) %>% prep(), NA)
  expect_error(recipe(x_dt) %>% step_cut(cat_var, breaks = 2) %>% prep(),
               "All columns selected for the step should be numeric")
  expect_error(recipe(x_dt) %>% step_cut(everything(), breaks = 2) %>% prep(),
               "All columns selected for the step should be numeric")
})
