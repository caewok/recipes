...
context("dtplyr: strings_as_factors conversion")
...
as_fact_dt <- lazy_dt(as_fact)
as_str_dt <- lazy_dt(as_str)

test_that('strings_as_factors = FALSE', {
  rec1 <- recipe(~ ., data = as_fact_dt) %>%
    step_center(numbers)
  rec1 <- prep(rec1, training = as_fact_dt,
                  strings_as_factors = FALSE, verbose = FALSE)
  rec1_as_fact <- bake(rec1, new_data = as_fact_dt)
  expect_warning(rec1_as_str <- bake(rec1, new_data = as_str_dt))
...
test_that('strings_as_factors = TRUE', {
  rec2 <- recipe(~ ., data = as_fact_dt) %>%
    step_center(numbers)
  rec2 <- prep(rec2, training = as_fact_dt,
                  strings_as_factors = TRUE, verbose = FALSE)
  rec2_as_fact <- bake(rec2, new_data = as_fact_dt)
  expect_warning(rec2_as_str <- bake(rec2, new_data = as_str_dt))
