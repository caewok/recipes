...
context("dtplyr: Indicator for novel levels")
...
tr_bad_dt <- lazy_dt(tr_bad)
te_miss_dt <- lazy_dt(te_miss)

iris_dt <- lazy_dt(iris)

rec <- recipe(~ ., data = tr_dat_dt)

test_that('basic functionality', {
  ex_1 <- rec %>%
    step_novel(all_predictors()) %>%
    prep(tr_dat_dt, strings_as_factors = FALSE)

  ex_1_tr <- bake(ex_1, new_data = tr_dat_dt)
  ex_1_te <- bake(ex_1, new_data = te_dat_dt)
...
test_that('missing values', {
  ex_2 <- rec %>%
    step_novel(all_predictors()) %>%
    prep(training  = tr_dat_dt)
  ex_2_te <- bake(ex_2, new_data = te_miss_dt)
...
test_that('printing', {
  ...
  expect_output(print(prep(ex_3, training = tr_dat_dt, verbose = TRUE)))
})
