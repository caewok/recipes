...
date_rec <- recipe(~ Dan + Stefan, examples_dt) %>%
...
test_that('default option', {
  ...
  date_rec <- prep(date_rec, training = examples_dt)
  date_res <- bake(date_rec, new_data = examples_dt)
...
test_that('printing', {
  ...
  expect_output(prep(date_rec, training = examples_dt, verbose = TRUE))
})
