...
context("dtplyr: Spare matrix format")
...
rec <- recipe( ~ ., data = okc_tr_dt) %>%
  step_modeimpute(all_nominal()) %>%
  step_meanimpute(all_numeric()) %>%
  step_dummy(location, diet) %>%
  prep(training = okc_tr_dt)

###################################################################

test_that('correct types', {
  bake_default <- bake(rec, new_data = okc_te_dt, all_numeric())
  bake_sparse <-
    bake(rec,
         new_data = okc_te_dt,
         all_numeric(),
...
test_that('bad args', {
  expect_error(bake(rec, new_data = okc_te_dt, composition = "dgCMatrix"))
...
test_that('issue 206 - NA values are kept when requesting matrix composition', {
  ...
  rec <- recipe(x2 ~ ., data = df_dt) %>%
    step_dummy(x) %>%
    prep(df_dt)

  res_mat <- bake(rec, df_dt, composition = "matrix")
  res_sparse <- bake(rec, df_dt, composition = "dgCMatrix")
