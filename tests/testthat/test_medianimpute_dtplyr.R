...
context("dtplyr: Median imputation")
...
credit_tr_dt <- lazy_dt(credit_tr)
credit_te_dt <- lazy_dt(credit_te)

test_that('simple median', {
  rec <- recipe(Price ~ ., data = credit_tr_dt)

  impute_rec <- rec %>%
    step_medianimpute(Age, Assets, Income, id = "")
  imputed <- prep(impute_rec, training = credit_tr_dt, verbose = FALSE)
  te_imputed <- bake(imputed, new_data = credit_te_dt)
...
test_that('non-numeric', {
  ...
  expect_error(prep(impute_rec, training = credit_tr_dt, verbose = FALSE))
})


test_that('printing', {
  impute_rec <- recipe(Price ~ ., data = credit_tr_dt) %>%
    step_medianimpute(Age, Assets, Income)
  expect_output(print(impute_rec))
  expect_output(prep(impute_rec, training = credit_tr_dt, verbose = TRUE))
})
