...
context("dtplyr: Creating profiling data")
...
test_that('character profile', {
  chr_rec <- okc_rec %>%
    ...
    prep(okc_dt, strings_as_factors = FALSE) %>%
    juice()
...
test_that('printing', {
  num_rec_1 <- okc_rec %>%
    step_profile(-age, profile = vars(age))
  num_rec_2 <- prep(num_rec_1, okc_dt)
...
test_that('tidy', {
  ...
  num_rec_4 <- prep(num_rec_3, okc_dt)
