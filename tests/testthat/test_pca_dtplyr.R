...
test_that('correct PCA values', {
  ...
  pca_extract_trained <- prep(pca_extract, training = biomass_tr_dt, verbose = FALSE)

  pca_pred <- bake(pca_extract_trained, new_data = biomass_te_dt, all_predictors())
...
test_that('printing', {
  ...
  expect_output(prep(pca_extract, training = biomass_tr_dt, verbose = TRUE))
})
...
test_that('No PCA comps', {
  ...
  pca_extract_trained <- prep(pca_extract, training = biomass_tr_dt)
  expect_equal(
