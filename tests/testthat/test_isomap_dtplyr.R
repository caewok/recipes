...
dat1_dt <- lazy_dt(dat1)
dat2_dt <- lazy_dt(dat2)

rec <- recipe( ~ ., data = dat1_dt)
...
test_that('correct Isomap values', {
  ...
  im_trained <- prep(im_rec, training = dat1_dt, verbose = FALSE)
...
test_that('printing', {
  ...
  expect_output(prep(im_rec, training = dat1_dt, verbose = TRUE))
})
...
test_that('ISOmap fails gracefully', {
  ...
  expect_error(
    recipe(Sepal.Length ~ ., data = iris_dt) %>%
