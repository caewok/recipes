...
test_that("in recipe", {
  ...
  train_dt <- lazy_dt(train)
  test_dt <- lazy_dt(test)

  rec1  <- recipe(train_dt) %>% check_range(x, y, slack_prop = 0.2) %>% prep()
  expect_error(bake(rec1, test_dt), NA)
  expect_warning(bake(rec1, test_dt), NA)

  rec2 <- recipe(train_dt) %>% check_range(x, y) %>% prep()
  expect_error(bake(rec2, test_dt),
               "min x is -10, lower bound is -5, max x is 110, upper bound is 105")

  rec3 <- recipe(train_dt) %>% check_range(x, y, warn = TRUE) %>% prep()
  expect_warning(bake(rec3, test_dt),
                 "min x is -10, lower bound is -5, max x is 110, upper bound is 105")

  rec4 <- recipe(train_dt) %>% check_range(y, slack_prop = c(0.2, 0.1)) %>% prep()
  expect_error(bake(rec4, test_dt),
...
test_that('printing', {
  ...
  check_range_extract <- recipe(mtcars_dt) %>%
    check_range(drat, cyl, am)
  expect_output(print(check_range_extract))
  expect_output(prep(check_range_extract, training = mtcars_dt, verbose = TRUE))
})
