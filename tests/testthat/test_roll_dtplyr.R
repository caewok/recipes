...
context("dtplyr: Rolling features")
...
sim_dat_dt <- lazy_dt(sim_dat)

rec <- recipe( ~ ., data = sim_dat_dt)
...
test_that('error checks', {
  ...
  expect_error(prep(rec %>% step_window(fac), training = sim_dat_dt))
  expect_error(prep(rec %>% step_window(y1, size = 1000L), training = sim_dat_dt))
  bad_names <- rec %>%
    step_window(starts_with("y"), names = "only_one_name")
  expect_error(prep(bad_names, training = sim_dat_dt))
...
test_that('basic moving average', {
  ...
  simple_ma <- prep(simple_ma, training = sim_dat_dt)
  simple_ma_res <- bake(simple_ma, new_data = sim_dat_dt)
...
test_that('creating new variables', {
  ...
  new_names <- prep(new_names, training = sim_dat_dt)
  new_names_res <- bake(new_names, new_data = sim_dat_dt)
...
test_that('printing', {
  ...
  expect_output(prep(new_names, training = sim_dat_dt, verbose = TRUE))
})
