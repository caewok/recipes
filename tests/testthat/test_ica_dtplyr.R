...
test_that('correct ICA values', {
  ...
  set.seed(12)
  ica_extract_trained <- prep(ica_extract, training = biomass_tr_dt, verbose = FALSE)

  ica_pred <- bake(ica_extract_trained, new_data = biomass_te_dt, all_predictors())
...
test_that('printing', {
  ...
  expect_output(prep(ica_extract, training = biomass_tr_dt, verbose = TRUE))
})
...
test_that('No ICA comps', {
  ...
  ica_extract_trained <- prep(ica_extract, training = biomass_tr_dt)
  expect_equal(
