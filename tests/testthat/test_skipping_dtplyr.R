...
test_that('simple skip', {
  rec_1 <- recipe(Sepal.Length ~ ., data = iris_dt) %>%
    step_log(Sepal.Length, skip = TRUE) %>%
    step_dummy(Species) %>%
    step_center(all_predictors())

  prepped_1 <- prep(rec_1, training = iris_dt)

  juiced_1 <- juice(prepped_1)
  baked_1  <- bake(prepped_1, new_data = iris_dt)
  ...
  expect_warning(prep(rec_1, training = iris_dt, retain = FALSE))
})
...
test_that('skips for steps that remove columns (#239)', {
  ...
  prep_simple <- prep(simple_ex, iris_dt)
  simple_juiced <- juice(prep_simple)
  simple_baked <- bake(prep_simple, new_data = iris_dt)
  ...
  complex_baked <- bake(complex_ex, new_data = iris_dt)
  ...
  iris_dups_dt <- lazy_dt(iris_dups)

  expect_warning(corr_example <-
    recipe(Species ~ ., data = iris_dups_dt) %>%
    step_corr(all_predictors(), skip = TRUE) %>%
    prep(), "Correlation copies the dtplyr table to a matrix")

  corr_juiced <- juice(corr_example)
  corr_baked <- bake(corr_example, new_data = iris_dups_dt)
