...
test_that('numeric data', {
  rec1 <- recipe(y ~ ., data = dat_dt) %>%
    step_shuffle(all_numeric())

  rec1 <- prep(rec1, training = dat_dt, verbose = FALSE)
  set.seed(7046)
  dat1 <- bake(rec1, dat_dt, all_predictors())
...
test_that('nominal data', {
  rec2 <- recipe(y ~ ., data = dat_dt) %>%
    step_shuffle(all_nominal())

  rec2 <- prep(rec2, training = dat_dt, verbose = FALSE)
  set.seed(804)
  dat2 <- bake(rec2, dat_dt, all_predictors())
...
test_that('all data', {
  rec3 <- recipe(y ~ ., data = dat_dt) %>%
    step_shuffle(everything())

  rec3 <- prep(rec3, training = dat_dt, verbose = FALSE)
  set.seed(2516)
  dat3 <- bake(rec3, dat_dt, all_predictors())
...
test_that('printing', {
  ...
  expect_output(prep(rec3, training = dat_dt, verbose = TRUE))
})

test_that('bake a single row', {
  rec4 <- recipe(y ~ ., data = dat_dt) %>%
    step_shuffle(everything())
