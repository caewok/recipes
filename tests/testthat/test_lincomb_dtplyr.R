...
context("dtplyr: Linear combination detection")
...
dummies_dt <- lazy_dt(dummies)

dum_rec <- recipe(yield ~ . , data = dummies_dt)
...
test_that('example 1', {
  ...
  dum_filtered <- prep(dum_filtered, training = dummies_dt, verbose = FALSE)
  removed <- c("N1", "P1", "K1")
...
test_that('example 2', {
  ...
  filtering_trained <- prep(lincomb_filter, training = biomass_tr_dt)
  test_res <- bake(filtering_trained, new_data = biomass_te_dt, all_predictors())

  expect_true(all(!(paste0("new_", 1:2) %in% colnames(test_res))))
})

test_that('no exclusions', {
  biomass_rec_2 <- recipe(HHV ~ carbon + hydrogen, data = biomass_tr_dt)
  lincomb_filter_2 <- biomass_rec_2 %>%
    step_lincomb(all_predictors())

  filtering_trained_2 <- prep(lincomb_filter_2, training = biomass_tr_dt)
  test_res_2 <- bake(filtering_trained_2, new_data = biomass_te_dt, all_predictors())
...
test_that('printing', {
  ...
  expect_output(prep(dum_filtered, training = dummies_dt, verbose = TRUE))
})
