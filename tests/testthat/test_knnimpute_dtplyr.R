...
context("dtplyr: K-nn imputation")
...
test_that('imputation values', {
  ...
  discr_rec <- prep(discr_rec, training = biomass_tr_dt, verbose = FALSE)
  tr_data <- bake(discr_rec, new_data = biomass_tr_dt)
  te_data <- bake(discr_rec, new_data = biomass_te_dt) %>%
  ...
  impute_rec <- prep(impute_rec, training = biomass_tr_dt, verbose = FALSE)
  imputed_te <- bake(impute_rec, new_data = biomass_te_dt)
...
test_that('printing', {
  ...
  expect_output(prep(discr_rec, training = biomass_tr_dt, verbose = TRUE))
})
