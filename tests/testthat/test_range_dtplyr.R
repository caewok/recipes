...
test_that('correct values', {
  ...
  standardized_trained <- prep(standardized, training = biomass_tr_dt, verbose = FALSE)

  obs_pred <- bake(standardized_trained, new_data = biomass_te_dt, all_predictors())
...
test_that('one variable', {
  ...
  obs_pred <- bake(standardized_trained, new_data = biomass_te_dt)
...
test_that('printing', {
  ...
  expect_output(prep(standardized, training = biomass_tr_dt, verbose = TRUE))
})
