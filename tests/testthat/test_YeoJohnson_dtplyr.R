...
test_that('missing data', {
  ex_dat$x1[1] <- NA
  rec_true <- recipe(~., data = ex_dat_dt) %>%
    step_YeoJohnson(x1, x2, x3, x4)

  expect_error(prep(rec_true, training = ex_dat_dt, verbose = FALSE), NA)

  rec_false <- recipe(~., data = ex_dat_dt) %>%
    step_YeoJohnson(x1, x2, x3, x4, na.rm = FALSE)

  expect_error(prep(rec_false, training = ex_dat_dt, verbose = FALSE))
})
